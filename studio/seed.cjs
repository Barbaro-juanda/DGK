// Carga los 9 productos + textos usando la SESIÓN de Sanity ya iniciada (no necesita token manual).
// Se corre con:  cd studio && npx sanity exec seed.cjs --with-user-token
...
async function main() {
  ...
  console.log(`Listo: ${productos.length} productos + textos cargados. Documentos: ${res.results.length}`);
}
