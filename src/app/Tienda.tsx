...
import { wa } from "./wa";
import { products, categorias } from "./products";
...
export default function Tienda() {
  ...
  useEffect(() => {
    ...
    document.title = "Tienda DGK — Productos y precios | Cerámico, shampoo, lubricante y más";
    metaEl?.setAttribute(
      "content",
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray<HTMLElement>("[data-stagger]").forEach((group) => {
        gsap.fromTo(group.children, { opacity: 0, y: 18 }, { opacity: 1, y: 0, stagger: 0.06, duration: 0.55, ease: "power3.out", scrollTrigger: { trigger: group, start: "top 90%", once: true } });
      });
    }, app);
    return () => ctx.revert();
  }, []);
      ...
      <main id="contenido">
        <section className="bg-[#121210] px-5 pb-14 pt-32 md:px-10 md:pb-16 md:pt-40">
          <div className="mx-auto max-w-[1480px]">
            <p className="mb-6 flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.24em] text-[#E5B500]"><span className="size-1.5 rounded-full bg-[#E5B500]" /> Tienda DGK / Productos</p>
        ...
        {categorias.map((categoria) => {
          ...
          return (
            <section key={categoria} className="bg-[#171714] px-5 pb-16 pt-2 md:px-10 md:pb-20">
              <div className="mx-auto max-w-[1480px]">
                <p data-reveal className="mb-6 border-b border-white/12 pb-4 font-mono text-[11px] uppercase tracking-[0.2em] text-[#F2C623]">{categoria}</p>
                <div data-stagger className="grid gap-4 grid-cols-2 lg:grid-cols-3">
                  {items.map((p) => (
        ...
        <section className="bg-[#171714] px-5 pb-24 md:px-10 md:pb-32">
          <div className="mx-auto max-w-[1480px]">
